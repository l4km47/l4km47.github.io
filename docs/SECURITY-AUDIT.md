...
plus `<meta name="referrer" content="strict-origin-when-cross-origin">`. The only third-party hosts on the allowlist are Google Fonts (stylesheet + font files) and `avatars.githubusercontent.com` (the About page avatar). `'unsafe-inline'` is needed only for `style-src` (Vue inline style attributes and mermaid's in-SVG `<style>`); scripts get neither `unsafe-inline` nor `unsafe-eval`.
